...
//            list.forEach(m -> setmealNames.addAll(m.keySet()));
    ...
    @RequestMapping("/getMemberByTime")
    public Result getMemberByTime(@RequestBody Map<String, String> times) throws Exception {
        String start = times.get("start");
        String end = times.get("end");


        List<String> months = DateUtils.getMonthBetween(start, end, "yyyy-MM");
        Map<String, Object> map = new HashMap<>();
        map.put("months", months);
        List<Integer> memberCount = reportService.getMemberCountsByMonths(months);
        map.put("memberCount", memberCount);
    ...
    @RequestMapping("/getMemberByGender")
    public Result getMemberByGender() {
        try {
            List<Map<String, Object>> list = reportService.getMemberByGender();
            ...
            map.put("genderCount", list);
            List<String> genders = new ArrayList<>();
            list.forEach(m -> genders.add((String) m.get("name")));
//            list.forEach(m -> genders.addAll(m.keySet()));
    ...
    public Result getMemberByAge() {
        try {
            List<Map<String, Object>> list = reportService.getMemberByAge();
            Map<String, Object> map = new HashMap<>();
            map.put("ageCount", list);
            List<String> ageNames = new ArrayList<>();
            list.forEach(m -> ageNames.add((String) m.get("name")));
//            list.forEach(m -> setmealNames.addAll(m.keySet()));
